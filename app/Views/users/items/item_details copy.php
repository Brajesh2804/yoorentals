                        ...
                        if ($my_id):
                            ...
                            foreach ($messages as $m):
                                ...
                                <div class="flex <?= $isMine ? 'justify-end' : 'justify-start' ?>">
                                    <div
                                        class="<?= $isMine ? 'bg-blue-600 text-white rounded-tr-none' : 'bg-white border-slate-200 rounded-tl-none text-slate-800' ?> p-3 rounded-2xl max-w-[85%] shadow-sm relative border">
                    ...
                    <?php if (session()->get('userlogin')): ?>
                        <div class="relative">
                            <textarea id="messageText" rows="2" onkeydown="handleEnter(event)"
                                class="w-full border-2 p-3 pr-14 rounded-2xl focus:border-blue-500 outline-none transition-all resize-none bg-white"
                                placeholder="Type a message and press Enter..."></textarea>
                            <button onclick="sendMessage()"
                                class="absolute right-3 bottom-3 p-2.5 bg-blue-600 text-white rounded-xl hover:bg-slate-900 transition-all shadow-lg flex items-center justify-center">
...
<script>
    ...
    function sendMessage() {
        ...
        // --- LINE BADLI GAYI HAI (Current Time Logic) ---
        const now = new Date();
        const time = now.toLocaleString('en-US', { hour: '2-digit', minute: '2-digit', hour12: true });

        // --- LINE BADLI GAYI HAI (Dynamic Time inject kiya) ---
