                ...
                <div class="divide-y divide-slate-100">
                    <?php if (!empty($chat_list)):
                        usort($chat_list, function ($a, $b) {
                            $a_unread = $a->unread_count ?? 0;
                            $b_unread = $b->unread_count ?? 0;
                        ...
                        foreach ($chat_list as $chat):
                            ...
                            <a href="<?= base_url('messages/chat/' . $chat->ad_id . '/' . $chat->other_user_id) ?>"
                                ...
                                <div class="relative">
                                    <div
                                        class="w-12 h-12 <?= $isUnread ? 'bg-blue-700' : 'bg-slate-400' ?> rounded-full flex-shrink-0 flex items-center justify-center text-white font-bold shadow-sm">
                                ...
                                <div class="flex-1 min-w-0">
                                    <div class="flex justify-between items-baseline">
                                        <h3
                                            ...
                                            <?= $chat->user_name ?></h3>
                                        <span
                                            class="text-[10px] <?= $isUnread ? 'text-blue-600 font-black' : 'text-slate-400 font-bold' ?>"><?= date('h:i A', strtotime($chat->last_time)) ?></span>
                                    </div>
                                    <p class="text-[11px] text-blue-600 font-bold truncate">Ad: <?= $chat->ad_title ?></p>
                                    <p
                                        class="text-xs <?= $isUnread ? 'text-slate-900 font-bold' : 'text-slate-500' ?> truncate">
                                        <?= esc($chat->last_msg) ?></p>
                                </div>
                            </a>
                        <?php endforeach; else: ?>
            ...
            <div class="flex-1 flex flex-col bg-white">
                            ...
                            <div class="flex <?= $isMine ? 'justify-end' : 'justify-start' ?>">
                                <div
                                    ...
                                    <div class="flex items-center justify-end gap-1 mt-1">
                                        <span
                                            class="text-[9px] <?= $isMine ? 'text-blue-100' : 'text-slate-400' ?> font-bold uppercase"><?= date('h:i A', strtotime($m->created_at)) ?></span>
                                        <?php if ($isMine): ?>
                    ...
                    <div class="p-4 bg-white border-t">
                        <div class="flex items-center gap-2">
                            ...
                            <button onclick="sendReply()"
                                class="p-3 bg-blue-600 text-white rounded-xl hover:bg-slate-900 transition-all shadow-lg active:scale-95 flex items-center justify-center">
                ...
                <?php else: ?>
                    <div class="flex-1 flex flex-col items-center justify-center text-slate-400 p-10 text-center">
                        ...
                        <p class="text-sm mt-2 max-w-xs font-medium">Select a customer to start dealing.</p>
                    </div>
                <?php endif; ?>
            </div>
        </div>
...
<script>
    ...
    function sendReply() {
        ...
        // --- MATCHING CURRENT TIME Logic (Customer Side Fix) ---
        const now = new Date();
        const time = now.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', hour12: true });

        const html = `
            <div class="flex justify-end">
                <div class="bg-blue-600 text-white p-3 rounded-2xl rounded-tr-none max-w-[75%] shadow-sm border relative">
                    <p class="text-sm">${message}</p>
                    <div class="flex items-center justify-end gap-1 mt-1">
                        <span class="text-[9px] text-blue-100 font-bold uppercase">${time}</span>
